...
function generateTextGeometry(text: string, params: any, anchor: any) {
...
function TextAnimation(textGeometry: THREE.TextGeometry) {
    var bufferGeometry = new BAS.ModelBufferGeometry(textGeometry);

    var aAnimation: any = bufferGeometry.createAttribute('aAnimation', 2);
    var aEndPosition: any = bufferGeometry.createAttribute('aEndPosition', 3);
    var aAxisAngle: any = bufferGeometry.createAttribute('aAxisAngle', 4);
...
var utils = {
    extend: function (dst: { [x: string]: any; }, src: { [x: string]: any; }) {
        for (var key in src) {
            dst[key] = src[key];
        }
    ...
    ease: function (ease: { getRatio: (arg0: number) => number; }, t: number, b: number, c: number, d: number) {
...
function createTweenScrubber(tween: any, seekSpeed: number = 0.001) {

    function stop() {
        Tween.TweenMax.to(tween, 1, { timeScale: 0 });
    }

    function resume() {
        Tween.TweenMax.to(tween, 1, { timeScale: 1 });
    }
